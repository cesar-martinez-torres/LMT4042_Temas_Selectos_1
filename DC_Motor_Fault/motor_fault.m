%% DC Motor Fault Detection: Overload, Excessive Friction, and Stall
...
%% 2) Time Scenario with Faults
fs   = 1000;              % Hz simulation
Tend = 20;                % total seconds
dt   = 1/fs;

% Segments:
% 0–5 s: healthy
% 5–10 s: MECHANICAL OVERLOAD (tau_load increases)
% 10–15 s: EXCESSIVE FRICTION (b increases)
% 15–20 s: STALL (high tau_load)
seg = struct( ...
    't0', [0 5 10 15], ...
    't1', [5 10 15 20], ...
    'tau_load', [0.002 0.020 0.005 0.060], ... % Nm
    'b', [par.b0 par.b0 5*par.b0 par.b0] ...
);
...
ylabel('\omega (rad/s)'); title('Speed and Fault Events');
xregion([5 10], 'Overload');
xregion([10 15], 'Friction');
xregion([15 20], 'Stall');
